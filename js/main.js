...
async function submitRSVP() {
  ...
  if (!attendance) { showError('Please let us know if you will attend.'); return; }
  if (!event)      { showError('Please select which event(s) you will attend.'); return; }

  const payload = { name, email, phone, attendance, guests: attendance === 'no' ? 0 : parseInt(guests), event, diet, message };
